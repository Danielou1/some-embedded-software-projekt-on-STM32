 ...
 * @brief   Main program for the 05_Potis_DMA project.
 *
 * @note    This project demonstrates reading potentiometer values using the ADC
 *          in DMA mode. In this mode, the CPU is freed from actively managing
 *          ADC conversions, allowing it to perform other tasks while conversions
...
int main(void)
{
    ...
    // Initialize potentiometers in DMA mode.
    // In this mode, ADC conversions run continuously in the background,
    // and potis_get_val() will retrieve the latest averaged value from the DMA buffer.
    potis_init(POTI_MODE_DMA);
    ...
    while(1) {
        ...
        // Read potentiometer 1 value. In DMA mode, this reads from the continuously updated buffer.
        ...
        // In DMA mode, the CPU is largely free during this delay and during ADC conversions,
        // allowing it to perform other tasks concurrently.
        HAL_Delay(100);
    }
}
